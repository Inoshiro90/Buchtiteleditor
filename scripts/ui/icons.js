// editor/ui/icons.js
// Central SVG icon registry — all Lucide icons used in the editor.
// Usage: icon('trash-2')  → returns an <svg>...</svg> string (inline, currentColor)
// Size defaults to 16×16 for UI; pass size param to override.
...
export function icon(name, size = 16, cls = '') {
  ...
  return `<svg${clsAttr} xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
}

/** Convenience: 14px icon (toolbar buttons) */
export const icon14 = (name, cls) => icon(name, 14, cls);

/** Convenience: 12px icon (small inline) */
export const icon12 = (name, cls) => icon(name, 12, cls);
